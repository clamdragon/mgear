...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        # self.foot_normal = datatypes.Vector.zAxis * self.guide.atra[3]
        if self.settings["use_blade"]:
            self.foot_t = transform.getTransformLookingAt(
                self.guide.apos[3],
                self.guide.apos[4],
                self.blade_normal,
                "xz",
        ...
        self.legBonesFK[-1].setMatrix(self.foot_t, ws=True)
        ...
        self.legBonesIK[-1].setMatrix(self.foot_t, ws=True)
        ...
        if self.settings.get("invert_parent_scale"):
            ...
            applyop.invert_parent_scale(self.fk3_npo, self.fk2_ctl)
        ...
        attribute.setRotOrder(self.tws3_drv, "ZYX")
        if self.settings["use_blade"]:
            # works best aligned with calf, not whatever end eff/foot is doing
            self.tws3_drv.r.set(0, 0, 0)
        ...
        end_rot = transform.getTransform(self.legBones[3])
        if self.negate:
            end_rot = datatypes.EulerRotation(0, 0, 180).asMatrix() * end_rot
